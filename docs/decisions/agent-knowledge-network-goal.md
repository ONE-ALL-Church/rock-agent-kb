...
- Curate: local convention is encoded in `AGENTS.md`, and tracked private path prefixes are blocked by `scripts/audit_tracked_tree.py`.
- Vouch: tracked-tree audit, bundle validation, `kb audit all`, and CI public-surface checks exist; a separate gitleaks-style scan is still required before the first public push.
- Create and migrate: not complete. The current repository remains private and should not be flipped public in place.
- Harden before automating: partial. `.github/CODEOWNERS`, CI checks, local audit docs, and `docs/runbooks/github-public-repo-hardening.md` exist; GitHub secret scanning, push protection, branch protection, repository rulesets, deploy environment, and proof tests must be enabled on the fresh public repo.
- Retire split machinery: tracked export removal and public-surface audit reuse are complete.
