...
platforms, concept IDs, capability names, and public configuration identifiers.
It supports `--scope open|historical-unresolved|all-relevant`, `--limit`, and
`--offset`, and returns explicit evidence, exclusions, risk provenance,
freshness, and live-verification metadata. The published client's `issues
watch` command follows all assessment pages and writes an owner-only,
scope-specific private local baseline so later runs can report routing, risk,
and remediation changes; use `--no-write` to preview and `--reset` to replace
the baseline. `issues assemble` is maintainer-only
