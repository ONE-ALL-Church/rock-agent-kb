...
from .open_questions import *  # noqa: F401,F403
from .render import *  # noqa: F401,F403
from .manifest import *  # noqa: F401,F403
from . import _shared, sections, citations, dependencies, open_questions, render, manifest

_MODULES: tuple[ModuleType, ...] = (_shared, sections, citations, dependencies, open_questions, render, manifest)

_OWNER_BY_NAME: dict[str, ModuleType] = {}
_RAW_BY_NAME: dict[str, Any] = {}
for _module in _MODULES:
    for _name, _value in vars(_module).items():
        if _name.startswith("_") or inspect.ismodule(_value):
            continue
...
def _sync_public_overrides() -> None:
    for name, raw in _RAW_BY_NAME.items():
        ...
        is_default_wrapper = inspect.isfunction(raw) and public_value is _WRAPPER_BY_NAME.get(name)
