...
- `lava-capabilities.jsonl`, `lava-capability-summary.json`, `lava-contexts.jsonl`, `lava-context-version-diff.jsonl`, and `lava-context-summary.json` - public Lava capability, version-aware data-context, and typed root-change reference data.
...
- `rock-ideas.jsonl`, `rock-idea-relationships.jsonl`, `rock-idea-verification-queue.jsonl`, and `rock-idea-summary.json` - bounded Rock Community Ideas feature-gap and roadmap metadata, typed evidence links, and lifecycle claims prioritized for corroboration. Idea statuses and queue states remain routing signals, not approved product claims or release proof.
