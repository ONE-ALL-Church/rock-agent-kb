...
| AccountNumberSecured | yes |  |  | yes |  | Gets or sets hash of the Checking Account AccountNumber. Stored as a SHA1 hash so that it can be matched without being known Enables a match of a Check Account to Person ( or Persons if multiple persons share a checking account) can be made |
