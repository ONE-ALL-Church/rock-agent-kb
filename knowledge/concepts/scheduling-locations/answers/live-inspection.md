---
id: answer:scheduling-locations:live-inspection
concept_id: scheduling-locations
generated: true
artifact_level: answer
---

# What live Rock records should I inspect for Scheduling And Locations?

For Scheduling And Locations, inspect these live surfaces before changing production behavior: Rock version, Block settings, WorkflowType and Workflow records, DataView and report filters, Group, GroupType, Location, and Schedule records, Mobile shell version and mobile block settings, Registration instance and registrant records, Person, family, alias, and attribute records.

## Top Claims

- `claim:4c4098a035a5ca256bfe`
- `claim:07a75e5ff71510d708de`
- `claim:2a9844acce5ba6150dec`
- `claim:9ad17cb08b8955d0d3ec`
- `claim:9c8ce297c9c4a4cda982`
- `claim:32f0173b23a7d2c356c0`

## Distilled Claims

- `distilled-claim:8bfc82127ec1e32191da`
- `distilled-claim:cd3356f2b9d53eef9dee`

## Citations

- [3 Underrated Features Churches Are Overlooking | Ep 217](https://www.youtube.com/watch?v=edanHiYSDIM&t=386s) (`06:26`)
- [3 Underrated Features Churches Are Overlooking | Ep 217](https://www.youtube.com/watch?v=edanHiYSDIM&t=91s) (`01:31`)
- [3 Underrated Features Churches Are Overlooking | Ep 217](https://www.youtube.com/watch?v=edanHiYSDIM&t=195s) (`03:15`)
- [3 Underrated Features Churches Are Overlooking | Ep 217](https://www.youtube.com/watch?v=edanHiYSDIM&t=268s) (`04:28`)
- [Outreach Toolbox is Here in v19](https://www.youtube.com/watch?v=LNcx8t0mlQ4&t=64s) (`01:04`)
- [3 Underrated Features Churches Are Overlooking | Ep 217](https://shows.acast.com/rock-cast/episodes/3-underrated-features-ep-217) (`06:26`)
